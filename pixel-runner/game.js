...
document.addEventListener('keydown',event=>{const controls={arrowleft:'left',a:'left',arrowright:'right',d:'right',arrowup:'jump',w:'jump',' ':'jump',arrowdown:'down',x:'fire',f:'fire'},key=controls[event.key.toLowerCase()];if(key){event.preventDefault();press(key,true)}if(event.key.toLowerCase()==='p'&&running){paused=!paused;paused?showOverlay('PAUSED','遊戲暫停','按 P 繼續冒險。','繼續'):hideOverlay()}});
document.addEventListener('keyup',event=>{const controls={arrowleft:'left',a:'left',arrowright:'right',d:'right',arrowup:'jump',w:'jump',' ':'jump',arrowdown:'down',x:'fire',f:'fire'},key=controls[event.key.toLowerCase()];if(key)press(key,false)});
